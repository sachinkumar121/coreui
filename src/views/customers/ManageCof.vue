...
<div>
    ...
    <CModal :visible="show_pay_modal">
        <CModalHeader>
            <CModalTitle>Manage Cards on File</CModalTitle>
        </CModalHeader>
        <CModalBody>
        <div class="row">
                <div class="col-md-6" color="danger">
                        <label class="control-label">
                            <b>Select card on file: </b>&nbsp; 
                        </label>
        ...
        <CModalFooter>
            ...
            <CButton @click="addNewCard" color="info" class="mr-auto">Add new card</CButton>
            <CButton  @click="deleteCof" color="danger" class="mr-auto">Delete Selected Card</CButton>
          
            <CButton  @click="closePayModal()" color="success">OK</CButton>
        </CModalFooter>
    </CModal>
        ...
        methods: {
            ...
            addNewCard()
            {
                ...
                let url =  this.$store.state.skysched_global_api_base_url +
                            "extpages/add_payment_method/" +
                           this.$store.state.business_settings.api_access_account_id +
                            "?show_page_header=0&show_page_footer=0&customer_user_id=" +  this.selected_customer_id
                ...
                if (cardobj)
                {
                    ...
                    if (c)
                    {
                                    ...
                                    }).then((result)=>
                        {
                            if(result.status ==1)
                            {
                                this.takePayment({customer_id: this.selected_customer_id})
                                 this.closePayModal();
                            }
                            else
                            {
