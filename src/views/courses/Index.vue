	...
	<div class="pt-2">
        <FilterSection>
            ...
            <template v-slot:actions>
                <button class="btn btn-success float-end" @click="$router.push('/courses/add')">Create a Course</button>
            </template>
        </FilterSection>
				...
				<tbody v-if="list.length" >
					<tr v-for="item in list" :key="item._id">
						...
						<td class="action_column">
                            <button class="btn btn-primary btn-sm" @click="$router.push(`/courses/${item._id}/edit`)">Edit</button>
                            <button type="button" 
...
export default {
    ...
    methods: {
        ...
        deleteHandler(item)
        {
            ...
            if (confirmed) {
                this.$schedsdk.delete_course(item._id).then((result)=>{
                    this.getCoursesList();
                }).catch(console.error);
            }
        }
