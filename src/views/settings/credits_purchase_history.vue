<template>
    <div>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody v-if="list.length">
                    <tr v-for="(pline, index) in list" :key="index">
                        <td>{{ Math.abs(pline.credits_consumed) }}</td>
                        <td>{{ getFormattedDateTime(pline.created_at) }}</td>

                        <td>
                            <!-- <button class="btn btn-danger btn-sm" 
                                @click="cancelHandler(pline)">Cancel</button> &nbsp;
                             <button class="btn btn-success btn-sm" 
                                @click="purchaseOnce(pline)">Purchase once</button> &nbsp; -->
                            <!-- Implement Later -->
        ...
        <Pagination class="mt-2"
            v-if="list.length"
            :size="'sm'"
            :ariaLabel="'Credit purchase histories results pages'"
...
export default {
    ...
    methods: {
        doGetItemList(context) {
            this.$schedsdk
                .getMessageCreditsPurchaseHistory({})
                .then((result) => {
                    if (result.status == 1) {
