...
<div>
     <div class="col-md-12 text-right">
        <button class="btn btn-success"  @click="$router.push('/paperwork/manage_paperwork_requirements')"> Manage Paperwork Requirements </button>
    </div>
    <br>
    <CRow>
      <CCol sm="12">
       ...
       this.$schedsdk.get_short_ak_for_token(null).then(async (result)=>{
            ...
            if (result.status == 1)
            {
                this.iframe.src = this.$store.state.skysched_global_api_base_url + 
                                "v1/ext/single_sign_on_unauth/show_lagw_list/"+ 
                                result.data;
                this.iframe.style = {
                position: 'absolute',
