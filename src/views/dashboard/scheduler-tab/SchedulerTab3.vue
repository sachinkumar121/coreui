  ...
  //SCHEDULER_VIEW_PREFS
    ...
    methods : {
       ...
       refreshStartEnd(start,end)
      {
        ...
        getData() {
            ...
            if (user_type != 1 && user_type != 3) {
                ...
                userListPromise = this.schedapisdkref.get_users_list({});
            }
      ...
      getNonBookingEvents(date) {
           ...
           let that = this;
           this.schedapisdkref.getNonBookingProviderOccupancyList(data).then((result)=>{
                let events = [];
                let mbevents = [];
          if (result.status === 1) {
            for (let i = 0; i < result.data.length; i++) {
              ...
              if (result.data[i].type) {
                            ...
                            //console.log("--asdsaeasd",booking_helper.provider_occupied_record_to_mobiscroll(business_context,result.data[i]))
      ...
      getEvents(date,start,end) {
           ...
           this.schedapisdkref.getBookingList({begin_booking_date:start_day, end_booking_date:end_day}).then((result)=>{
                let events = [];
                let mbevents = [];
    ...
    mounted() {
            ...
            //console.log("asdasdsadasd0s0dad",  that.getDefaultViewObj())
               that.mbEvCal = mobiscroll.eventcalendar('#mycalendar', 
                {
                    view: that.getDefaultViewObj(),
                    dragToMove: true,
              ...
              renderDay: function (args) {
               ...
               renderResourceHeader: function () {
                  return '<div class="header-resource-content" >' +
