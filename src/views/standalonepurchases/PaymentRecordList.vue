  ...
  <div class="pt-2">
    ...
    <div class="table-responsive">
          ...
          <tbody> 
            <tr v-if='payments.length'>
                <template v-for="(pline, index) in payments" :key="index">
                        ...
                        <td v-else-if="pline.payment_txn_type == 2">Cheque</td> 
                        <td v-else>Other</td> 
                        ...
                        <td>{{getLimitedTxnNum(pline.payment_txn_number) }}</td> 
                        <td><a href="javascript:void(0)" @click="$router.push('/invoices/detail/'+  pline.invoice_number)">{{ pline.invoice_number }}</a></td> 
                        ...
                        <td>
                            <button class="btn btn-primary btn-sm" 
                                ...
                                &nbsp;
                            <button class="btn btn-primary btn-sm" 
                                @click="refundPayment(pline.invoice_number, pline._id)">Refund</button>
                        </td> 
                  </template>
                </tr>  
              <tr v-else><td colspan="10" class="text-center">No Invoice Exists</td></tr>
          </tbody>
      </table>
    </div>
        <Pagination class="mt-2"
            ...
            :size="'sm'"
            :ariaLabel="'payment records results pages'"
...
export default {
  ...
  methods:{
    doGetInvoicePaymentList(context){
        ...
        let data = {page_no:context.page, 
                    txn_ref_num: context.txn_ref_num ,
                    last4 : context.last4,
                    invoice_number: context.invoice_number }
        context.$schedsdk.get_invoices_payments_list(data).then((result)=>{
    ...
    refundPayment(invnum, payment_id)
    {
        console.log(invnum);
